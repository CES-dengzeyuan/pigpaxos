...
func (c *Config) Load() {
	...
	c.Addrs = make(map[ID]string, len(c.AddrsStr))
	for idStr, ip := range c.AddrsStr {
		c.Addrs[NewIDFromString(idStr)] = ip
	}

	c.HTTPAddrs = make(map[ID]string, len(c.HTTPAddrsStr))
	for idStr, httpaddress := range c.HTTPAddrsStr {
		c.HTTPAddrs[NewIDFromString(idStr)] = httpaddress
	}
